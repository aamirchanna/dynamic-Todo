...
function displayUserNotes() {
  ...
  if (notes) {
    ...
    notes.forEach(function (data, ind) {
      ...
      if (currentUserEmail === "admin@gmail.com" || data.email === currentUserEmail || data.email === localStorage.getItem("newEmail")) {
        var liElement = ` <li class="border rounded p-2 my-2">
        <p class = "font-medium">${data.note}</p> 
            <p>${data.email}</p>
            <button class="btn bg-green-600  text-white  px-3 rounded-md focus:outline-none onClick="deleteTodo">Delete</button>
...
function deleteTodo(index){
   var del = JSON.parse(getItem("notes")) 
   del.splice(index, 1);
   localStorage.setItem("notes", JSON.stringify(notes)); 
   displayUserNotes()
}
